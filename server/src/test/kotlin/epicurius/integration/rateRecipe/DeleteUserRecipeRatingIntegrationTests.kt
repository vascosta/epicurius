...
class DeleteUserRecipeRatingIntegrationTests : RateRecipeIntegrationTest() {

    @Test
    fun `Should delete user's recipe rating successfully with code 204`() {
        ...
        val response = getRecipeRating(token, testRecipe.id)
        assertNotNull(response)
        assertEquals(RATING_5.toDouble(), response?.rating)

        // and deletes the user's recipe rating
        deleteUserRecipeRating(token, testRecipe.id)

        // then the recipe rating is deleted successfully
        val rsp = getRecipeRating(token, testRecipe.id)
        assertNotNull(rsp)
    ...
    fun `Should fail with code 404 when trying to delete a recipe's rate with non existing recipe id`() {
        ...
        // when the user tries to delete the recipe's rating
        ...
        // then the user's recipe rating cannot be deleted and fails with code 404
    ...
    fun `Should fail with code 403 when recipe's author tries to delete recipe rate`() {
        ...
        // then the user's recipe rating cannot be deleted and fails with code 403
        val bodyError = getBody(error)
    ...
    fun `Should fail with code 400 when trying to delete a recipe's rate that has not been rated`() {
        ...
        // then the user's recipe rating cannot be deleted and fails with code 400
        val bodyError = getBody(error)
