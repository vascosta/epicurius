...
class UpdateUserRecipeRatingIntegrationTests : RateRecipeIntegrationTest() {
    ...
    fun `Should update recipe rate successfully with code 204`() {
        ...
        // and updates the recipe rating
        val rsp = updateUserRecipeRating(token, testRecipe.id, RATING_5)
    ...
    fun `Should fail with code 404 when trying to update a recipe's rate with non existing recipe id`() {
        ...
        // when the user tries to update the recipe's rating
        ...
        // then the user's recipe rating cannot be updated and fails with code 404
    ...
    fun `Should fail with code 403 when recipe's author tries to update recipe rate`() {
        ...
        // when the user tries to update their own recipe's rating
        val error = patch<Problem>(
            client,
        ...
        // then the user's recipe rating cannot be updated and fails with code 403
        val bodyError = getBody(error)
    ...
    fun `Should fail with code 400 when trying to update user's rate that does not exist`() {
        ...
        // when the user tries to update their recipe rating
        val error = patch<Problem>(
            client,
            api(Uris.Recipe.RATE_RECIPE.replace("{id}", testRecipe.id.toString())),
            body = mapOf(
        ...
        // then the user's recipe rating cannot be updated and fails with code 400
        val bodyError = getBody(error)
