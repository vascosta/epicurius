...
class UpdateFridgeProductControllerTests : FridgeHttpTest() {
    ...
    fun `Should update product quantity successfully`() {
        ...
        val response = updateProduct(testAuthenticatedUser, ENTRY_NUMBER, updateInput)

        // then the product is updated successfully
        assertEquals(HttpStatus.OK, response.statusCode)
        assertEquals(UpdateProductOutputModel(Fridge(listOf(updatedProduct))), response.body)
    }
    ...
    fun `Should update a product in user's fridge successfully`() {
        ...
        // when updating the product in the fridge
        val response = updateProduct(testAuthenticatedUser, ENTRY_NUMBER, updateProductInputModel)
    ...
    fun `Should throw ProductNotFound exception when updating a non-existing product`() {
        ...
        assertFailsWith<ProductNotFound> {
            updateProduct(testAuthenticatedUser, nonExistingEntryNumber, updateProductInputModel)
        }
    }
    ...
    fun `Should throw ProductIsAlreadyOpen when updating expiration date of an open product`() {
        ...
        // when trying to update the product in the fridge
        // then the product cannot be removed and throws ProductIsAlreadyOpen exception
        assertFailsWith<ProductIsAlreadyOpen> {
            updateProduct(testAuthenticatedUser, NEW_ENTRY_NUMBER, updateProductInputModel)
        }
    }
}
