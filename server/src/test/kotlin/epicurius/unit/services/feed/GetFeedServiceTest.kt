...
class GetFeedServiceTest : FeedServiceTest() {
    ...
    fun `Should retrieve user's empty feed`() {
        ...
        // mock
        whenever(jdbiFeedRepositoryMock.getFeed(USER_ID, pagingParams)).thenReturn(emptyList())
        whenever(pictureRepositoryMock.getPicture("", RECIPES_FOLDER)).thenReturn(null)

        // when retrieving the feed
        val feed = getFeed(USER_ID, pagingParams)
    ...
    fun `Should retrieve user's feed with recipes`() {
        ...
        // mock
        whenever(jdbiFeedRepositoryMock.getFeed(USER_ID, pagingParams)).thenReturn(listOf(jdbiRecipeInfo))
        whenever(pictureRepositoryMock.getPicture(jdbiRecipeInfo.pictures.first(), RECIPES_FOLDER)).thenReturn(ByteArray(0))
    ...
    fun `Should retrieve user's feed order by most recent recipe`() {
        ...
        // mock
        whenever(jdbiFeedRepositoryMock.getFeed(USER_ID, pagingParams)).thenReturn(listOf(jdbiRecipeInfo2, jdbiRecipeInfo))
        whenever(pictureRepositoryMock.getPicture(jdbiRecipeInfo.pictures.first(), RECIPES_FOLDER)).thenReturn(ByteArray(0))
        whenever(pictureRepositoryMock.getPicture(jdbiRecipeInfo2.pictures.first(), RECIPES_FOLDER)).thenReturn(ByteArray(0))
