...
class GetFollowersServiceTests : UserServiceTest() {
    ...
    fun `Should retrieve the followers of an user successfully`() {
        ...
        whenever(jdbiUserRepositoryMock.getFollowers(publicTestUser.id, null, limit)).thenReturn(mockFollowers)

        // when retrieving the followers of the user
        val followers = getFollowers(publicTestUser.id, null, limit)
