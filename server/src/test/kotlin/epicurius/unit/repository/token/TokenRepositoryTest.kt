...
class TokenRepositoryTest : RepositoryTest() {
    ...
    fun `Should delete a token for an user and then create a new one successfully`() {
        ...
        deleteToken(testUser.user.id)

        // then the token is deleted successfully
        val notFoundUser = tm.run { it.userRepository.getUser(tokenHash = testUser.user.tokenHash) }
        ...
        createToken(newTokenHash, lastUsed, testUser.user.id)
