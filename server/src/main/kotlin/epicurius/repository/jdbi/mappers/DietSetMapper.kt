...
class DietSetMapper : ColumnMapper<Set<Diet>> {
    override fun map(rs: ResultSet, columnNumber: Int, ctx: StatementContext): Set<Diet> {
        val dbArray = rs.getArray(columnNumber)
        val intoleranceIdx = getArray<Int>(dbArray).toList()

        return intoleranceIdx.map { Diet.fromInt(it) }.toSet()
    }
}
