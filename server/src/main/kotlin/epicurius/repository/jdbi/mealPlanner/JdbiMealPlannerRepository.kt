...
class JdbiMealPlannerRepository(private val handle: Handle) : MealPlannerRepository {
    ...
    override fun createMealPlanner(userId: Int, date: LocalDate) {
        handle.createUpdate(
            """
        ...
        val list = handle.createQuery(
            """
                SELECT mp.date, mpr.meal_time, 
                       r.id, r.name, r.cuisine, r.meal_type, r.preparation_time, r.servings, r.pictures_names
    ...
    override fun checkIfMealPlannerAlreadyExists(userId: Int, date: LocalDate): Boolean =
        handle.createQuery(
            """
                SELECT COUNT(*)
